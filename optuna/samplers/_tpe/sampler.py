...
def _get_pruned_trial_score(trial: FrozenTrial) -> Tuple[float, float]:
...
def _split_pruned_trials(
    ...
    return sorted(trials, key=_get_pruned_trial_score)[:n_below]
...
def _calculate_weights_below_for_multi_objective(
    ...
    if n_below == 0:
        ...
        reference_point = np.maximum(1.1 * worst_point, 0.9 * worst_point)
        reference_point[reference_point == 0] = EPS
        hv = _compute_hypervolume(value_points, reference_point)
        indices_mat = ~np.eye(n_below).astype(bool)
        contributions = np.asarray(
            [
                hv - _compute_hypervolume(value_points[indices_mat[i]], reference_point)
