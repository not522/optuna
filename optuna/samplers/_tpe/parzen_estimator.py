...
class _ParzenEstimator:
    ...
    def _untransform(self, samples_array: np.ndarray) -> dict[str, np.ndarray]:
        res = {param: samples_array[:, i] for i, param in enumerate(self._search_space)}
    ...
    def _compute_sigmas(self, low, high, mus, parameters) -> np.ndarray:
        if parameters.multivariate:
            ...
            sigma = (
                SIGMA0_MAGNITUDE
                * max(len(mus), 1) ** (-1.0 / (len(self._search_space) + 4))
                * (high - low)
            )
            sigmas = np.full(shape=(len(mus),), fill_value=sigma)
        else:
            ...
            sigmas = sorted_sigmas[np.argsort(sorted_indices)][: len(mus)]
        ...
        if parameters.consider_magic_clip:
            ...
            n_kernels = len(mus) + 1  # NOTE(sawa3030): +1 for prior.
